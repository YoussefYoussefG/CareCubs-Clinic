...
const EditUser = ({ openModal, setOpenModal, user, setUser }: { openModal: boolean, setOpenModal: React.Dispatch<React.SetStateAction<boolean>>, user: User | undefined, setUser: React.Dispatch<React.SetStateAction<User | undefined>> }) => {
    ...
    const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        try {
            const response = await fetch(`${process.env.NEXT_PUBLIC_SERVER_NAME}/update/user/admin/${localStorage.getItem("userId")}?token=${localStorage.getItem("accessToken")}`, requestOptions);
                ...
                <div className="fixed top-0 left-0 w-full h-full bg-gray-400 bg-opacity-70 z-40 flex justify-center items-center">
                    <div className='max-w-md w-full mx-auto my-auto rounded-none md:rounded-2xl p-4 md:p-8 shadow-input bg-white dark:bg-black flex flex-col'>
                        ...
                        <form className="my-8 bg-white" onSubmit={handleSubmit}>
                            ...
                            >
                                {loading ? <CircularProgress color="warning" size={"1.3rem"} /> : <p>Update User Info</p>}
                                <BottomGradient />
                            </button>
