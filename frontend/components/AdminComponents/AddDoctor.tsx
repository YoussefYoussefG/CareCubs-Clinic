...
const AddDoctor = ({ openModal, setOpenModal }: { openModal: boolean, setOpenModal: React.Dispatch<React.SetStateAction<boolean>> }) => {
    ...
    const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        try {
            const response = await fetch(`${process.env.NEXT_PUBLIC_SERVER_NAME}/add/doctor/${localStorage.getItem("userId")}?token=${localStorage.getItem("accessToken")}`, requestOptions);
                        ...
                        <form className="my-8 bg-white" onSubmit={handleSubmit}>
                            ...
                            <div className="flex flex-col md:flex-row space-y-2 md:space-y-0 md:space-x-2 mb-4">
                                ...
                                <LabelInputContainer className="mb-4">
                                    ...
                                    <Input id="price" name="price" value={formData.price} onChange={handleChange} placeholder="" type="number" />
                                </LabelInputContainer>
                            </div>
                            <button
                            ...
                            >
                                {loading ? <CircularProgress color="warning" size={"1.3rem"} /> : <p>Add Doctor</p>}
                                <BottomGradient />
                            </button>
