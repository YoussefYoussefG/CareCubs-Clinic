...
const AddPatient = ({ openModal, setOpenModal }: { openModal: boolean, setOpenModal: React.Dispatch<React.SetStateAction<boolean>> }) => {
    ...
    const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        try {
            const response = await fetch(`${process.env.NEXT_PUBLIC_SERVER_NAME}/add/patient/${localStorage.getItem("userId")}?token=${localStorage.getItem("accessToken")}`, requestOptions);
